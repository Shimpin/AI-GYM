<template>
  <!-- 查询区 -->
  <div class="search">
    <el-form inline :model="coachSearchModel" label-width="90" label-position="right" ref="memberSearchForm">
      <el-form-item label="ID：" prop="id">
        <el-input v-model="coachSearchModel.id" placeholder="请输入教练ID"/>
      </el-form-item>

      <el-form-item label="手机号：" prop="phone">
        <el-input v-model="coachSearchModel.phone" placeholder="请输入教练手机号"/>
      </el-form-item>

      <el-form-item label="姓名：" prop="name">
        <el-input v-model="coachSearchModel.name" placeholder="请输入教练姓名"/>
      </el-form-item>
  ...
  <!-- 表格区 -->
  <div class="grid">
    <el-table :data="coach" stripe border @row-click="tblRowClick" ref="tblRef" class="data-grid">
      ...
      <el-table-column prop="wechat" label="微信" width="160" fixed/>
      <el-table-column prop="recommend" label="励志格言"  fixed/>
  ...
  <el-dialog v-model="coachDlgShow" :title="coachDlgTitle" width="600"
    ...
    <el-form label-width="100" label-position="right"
             :model="coachModel" :rules="rules" ref="memberFormRef">
      <el-row>
        <el-col :span="16">
          <el-form-item label="手机号：" prop="phone">
      ...
      <el-form-item label="励志格言：" prop="recommend">
        <el-input type="textarea" v-model="coachModel.recommend" :rows="4" placeholder="请输入励志格言"/>
      </el-form-item>
...
async function search(pageNo = 1, pageSize = 10) {
  ...
  let params = toRaw(coachSearchModel.value);//查询条件
  ...
  //console.log(resp);
  coach.value = resp.data.records;//教练数据
...
function add() {
  ...
  mode = "add";
  const now = new Date();
  coachModel.value.createTime = formatDate(now);
}
    ...
    if (valid) {
      ...
      api({
        ...
        if (resp.success) {
          ...
          search();
          ElMessage.success("保存教练信息成功");
        } else {
          ElMessage.error(resp.msg || "保存教练信息失败");
        }
      }).catch(resp => {
        ElMessage.error(resp.msg || "保存教练信息失败");
      });
...
function customUpload(options) {
  ...
  api({
    url: '/coach/avatar', // 假设上传接口是 /coach/avatar
    method: 'post',
    data: formData,
    headers: {
